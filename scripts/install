...
echo "Installing mandatory packages before installation..."
{
pkg --check-mirror upgrade -y
pkg in ncurses-utils -y
dpkg -L termux-exec
} &> /dev/null 2>&1;
...
run_with_spinner "Removing previous AltaeraAI files " bash -c "proot-distro remove altaera"
...
run_with_spinner "Installing initial files " bash -c "bash 'altaera_install_pt-2.sh'"
