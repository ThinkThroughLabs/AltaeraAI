#!/bin/bash

clear

# Terminal dimensions
LINES=$(tput lines)
COLUMNS=$(tput cols)
RESERVED=2
OUTPUT_HEIGHT=$((LINES - RESERVED))
SEPARATOR_LINE=$((LINES - 2))
FOOTER_LINE=$((LINES - 1))

# Create log file
LOGFILE=$(mktemp)

files=()
while IFS= read -r -d $'\0' file; do
    files+=("$file" "")
done < <(find "/root/models" -maxdepth 1 -type f -iname \*.gguf -print0)

if [ ${#files[@]} -eq 0 ]; then
    clear
    bash '/root/altaera-model_empty.sh'
else
    file=$(dialog --stdout --title "Select an AI Model:" --menu "Choose a file:" 0 0 0 "${files[@]}")
fi

RESULT=$?
if [ $RESULT -eq 0 ]; then

if [[ $file == *.gguf ]]; then
clear
cd kcpp-ae

# Start Python script
python3 koboldcpp.py "$file" 1551 \
--blasbatchsize 2048 \
--contextsize 2048 > "$LOGFILE" 2>&1 &
PYTHON_PID=$!

# Function to draw footer
FOOTER_MSG="KoboldCpp session initialized. To stop it, press 'q'."
draw_footer() {
    tput cup "$SEPARATOR_LINE" 0
    printf '%*s' "$COLUMNS" '' | tr ' ' '-'
    tput cup "$FOOTER_LINE" 0
    tput el
    PADDING=$(( (COLUMNS - ${#FOOTER_MSG}) / 2 ))
    printf "%*s\033[1m%s\033[0m" "$PADDING" '' "$FOOTER_MSG"
}

# Spinner thread while initializing
SPINNER_CHARS=('|' '/' '-' '\\')
spinner_running=true
spinner() {
    local i=0
    while $spinner_running; do
        SPIN_CHAR="${SPINNER_CHARS[i % 4]}"
        tput cup "$SEPARATOR_LINE" 0
        printf '%*s' "$COLUMNS" '' | tr ' ' '-'
        tput cup "$FOOTER_LINE" 0
        tput el
        INIT_MSG="KoboldCpp session initializing [ $SPIN_CHAR ] To stop it, press 'q'."
        PADDING=$(( (COLUMNS - ${#INIT_MSG}) / 2 ))
        printf "%*s\033[1m%s\033[0m" "$PADDING" '' "$INIT_MSG"
        sleep 0.2
        ((i++))
    done
}
spinner &
SPINNER_PID=$!

# Footer refresher to persist message
refresh_footer() {
    while true; do
        sleep 2
        draw_footer
    done
}
refresh_footer &
FOOTER_REFRESH_PID=$!

# Show log output
(
    tput civis
    LINE_COUNT=0
    tail -n +1 -F "$LOGFILE" | while IFS= read -r line; do
        if (( LINE_COUNT >= OUTPUT_HEIGHT )); then
            clear
            LINE_COUNT=0
        fi
        tput cup "$LINE_COUNT" 0
        tput el
        printf "%s\n" "$line"
        ((LINE_COUNT++))
    done
) &
DISPLAY_PID=$!

# Wait until ready
(
    while ! grep -q 'Please connect to custom endpoint at' "$LOGFILE"; do
        sleep 0.5
    done
    spinner_running=false
    wait "$SPINNER_PID" 2>/dev/null
    draw_footer
    termux-open-url 'http://localhost:1551/?streaming=1#'
) &
OPEN_PID=$!

# Wait for 'q'
while IFS= read -rsn1 key; do
    [[ $key == "q" ]] && break
done

# Cleanup
kill -15 "$PYTHON_PID" "$DISPLAY_PID" "$OPEN_PID" "$FOOTER_REFRESH_PID" 2>/dev/null
wait "$PYTHON_PID" 2>/dev/null
rm "$LOGFILE"
tput cnorm

# Clear footer
clear
echo "KoboldCpp session terminated. To start AltaeraAI again, type in 'ae'."
fi

else
clear
exit
'./data/data/com.termux/files/home/AltaeraAI/altaera.sh'
fi
